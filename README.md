...
主窗口由 Electron 28+ 拉起，React + Antd 5 渲染。在 `pnpm dev` 模式下 Vite 在 `http://localhost:5173` 提供热重载。

**主导航**（左侧栏）：
- **首页** — 资料总数 / 本月新增 / 即将到期 / 处理失败 / 最近活动
- **导入** — 拖拽文件或文件夹、选择目录、暂停 / 恢复 / 取消、实时进度
- **资料管理** — 搜索（高亮匹配位置）、多选、PDF/图片预览、批量重试 OCR、批量设置分类
- **回收站** — 30 天软删除，可恢复 / 永久删除 / 一键清空
- **设置** — 分类管理（含色标 + 拖拽重排 + 测试匹配）、到期提醒、自动备份（含保留策略）、备份目录、导入策略、OCR 引擎、外观（浅色 / 深色 / 跟随系统）

**关键 UX 细节**：
- 暗色主题随系统切换，并在用户偏好中持久化
- 关键信息（证书号 / 企业名称 / 有效期）从 OCR 文本自动提取，并在修正后自动重新分类
- 分类色标同时用于树形图标的文件夹颜色和右侧详情面板的 ColorPicker
- 搜索使用 SQLite FTS5（带中文 unicode61 分词），命中位置在结果中高亮显示
- 即将到期资料通过主进程定时器在 09:00 提醒，并通过 in-app 通知中心收件
